...
public class NSVM extends GraphicsProgram{
	...
	public static Integer[] Menu_Choices;
...
//		System.out.println(kernel(new Matrix(x, 4), new Matrix(y, 4)));
	...
	public void init() {
		...
		int x = Integer.parseInt(JOptionPane.showInputDialog("How many types of Data?"));
		Menu_Choices = new Integer[x];
		for(int i = 1; i <= x; i++){
			Menu_Choices[i-1] = i;
		}
		colors = new Color[x];
...
//			}
	...
	private void addPolygon(int x, int y) {
		if(currentType == 0){
			GOval o = new GOval(x - OBJECT_RADIUS, y - OBJECT_RADIUS, 2 * OBJECT_RADIUS, 2 * OBJECT_RADIUS);
			o.setFillColor(colors[0]);
			o.setFilled(true);
			add(o);
			return;
		}
		int sides = currentType + 2;
		GPolygon p = new GPolygon(x + OBJECT_RADIUS, y + OBJECT_RADIUS);
		double degrees = 360 / sides;
		for(int i = 0; i < sides; i++){
			p.addPolarEdge(2 * OBJECT_RADIUS, (i + 1) * degrees);
		}
		p.setFillColor(colors[currentType]);
		p.setFilled(true);
		add(p);
	}
...
//			System.out.println("done");
	...
	private void findNumType(ArrayList<Integer> yVals2) {
